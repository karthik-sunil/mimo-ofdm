...
function fft_file_processor(input_file, output_file)
    ...
    % Input:
    %   input_file  - Path to the input file with 'real,imaginary' values in 16,8 format
    %   output_file - Path to the output file for FFT outputs

    %input file
    data = readmatrix(input_file, 'Delimiter', ',', 'OutputType', 'double');
    real_part = data(:, 1)/256; % Convert to floating-point
    imag_part = data(:, 2)/256; % Convert to floating-point
    ...
    for chunk = 1:num_chunks
        ...
        if end_idx - start_idx + 1 < chunk_size
            % Pad with zeros if the chunk is smaller than 8
...
function X = dif_fft_complex_verbose(real_part, imag_part)
    ...
    for stage = 1:num_stages
        ...
        for k = 1:length(twiddles)
            realPart = real(twiddles(k));
            imagPart = imag(twiddles(k));
            fixedReal = round(realPart * 2^8); % Convert to 16,8 fixed-point
            fixedImag = round(imagPart * 2^8); % Convert to 16,8 fixed-point
    
            fprintf('\nTwiddle[%d]: Decimal: (%.6f, %.6f), Fixed-Point: (%d, %d)\n', k-1, realPart, imagPart, fixedReal, fixedImag);
        end
...
%input file and outputfile to be added here as parameters
% here is an example to run it
fft_file_processor('modulated.txt', 'modulatedgolden.txt');
