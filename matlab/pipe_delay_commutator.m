function fft_reorder(N)
    ...
    %using cell arrays to print ||
    top_row = num2cell(0:(N/2 - 1));
    bottom_row = num2cell((N/2):(N - 1));

    % Inline function to format a row without cell array markers
    format_row = @(row) strjoin(cellfun(@num2str, row, 'UniformOutput', false), ' ');
    ...
    while delay >= 1
        ...
        top_row = [repmat({"||"}, 1, delay), top_swapped];
